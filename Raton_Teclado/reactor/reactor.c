...
static void reactor_dispatch_event (reactor* r1, reactor* r2, fd_set* fds);

void reactor_run (reactor* r1,eactor* r2)
{
    for(r1->running = 1; r1->running; )
        reactor_demultiplex_events(r1,r2);
}
...
static void reactor_dispatch_event (reactor* r1, reactor* r2, fd_set* fds)
{
    //for (int fd = 0; fd <= r->max_fd; ++fd)
     ...
     if (FD_ISSET (r1->max_fd, &fds_copy))
     {
		 reactor_try_run_handler(r1,r1->max_fd);
		 
	 }      
	 if (FD_ISSET (r2->max_fd, &fds_copy))
     {
		 reactor_try_run_handler(r2,r2->max_fd);
...
void reactor_demultiplex_events (reactor* r1,reactor* r2)
{
	r1->fds=r2->fds;
    fd_set fds_copy = r1->fds;
    int ret = select(r2->max_fd+1, &fds_copy, 0, 0, &r2->tv);
    assert(ret >= 0);
